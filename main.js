...
function currentHotkeys() {
  return { ...DEFAULT_HOTKEYS, ...(uiState.hotkeys || {}) };
}
...
function registerHotkeys() {
  ...
  let show = false,
    clickThrough = false;
  try {
    show = globalShortcut.register(hk.show, toggleShow);
  } catch (e) {
    show = false;
  }
  try {
    clickThrough = globalShortcut.register(hk.clickThrough, toggleClickThrough);
  } catch (e) {
    clickThrough = false;
  }
...
function boundsVisible(b) {
  ...
  return screen.getAllDisplays().some((d) => {
    const wa = d.workArea;
    return b.x < wa.x + wa.width && b.x + Math.min(b.width, 200) > wa.x && b.y < wa.y + wa.height && b.y + 40 > wa.y;
  });
}
...
function toggleClickThrough() {
  ...
  uiState.clickThrough = clickThrough;
  saveState();
...
function main() {
  app.whenReady().then(() => {
    ...
    const okShow = reg.show,
      okClick = reg.clickThrough;
    if (!okShow || !okClick) {
      ...
      detectSiblingPalPocket((siblingRunning) => {
        ...
        const notify = () => {
          if (win) win.webContents.send("hotkeys-unavailable", { show: okShow, clickThrough: okClick });
        };
    ...
    ipcMain.on("open-external", (_e, url) => {
      ...
      const ok =
        /^https:\/\/paldb\.cc\//.test(url) || /^https:\/\/github\.com\/Cloudjunkie-Tobias\/PalPocket(\/|$)/.test(url);
